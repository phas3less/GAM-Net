...
from utils.dataloader import My_get_loader,My_test_dataset
...
def test(model, data_root):

    # data_path = os.path.join(path, dataset)
    # image_root = '{}/images/'.format(data_path)
    # gt_root = '{}/masks/'.format(data_path)
    model.eval()
    test_loader = My_test_dataset(data_root, 352)
...
def train(train_loader, model, optimizer, epoch, data_root):
    model.train()
    global best
    ...
    # torch.save(model.state_dict(), save_path +str(epoch)+ 'AntrumPVT.pth')
    ...
    if (epoch + 1) % 1 == 0:
        ...
        dataset_dice = test(model, data_root)
        logging.info('epoch: {}, dice: {}'.format(epoch, dataset_dice))
        if dataset_dice > best:
            best = dataset_dice
            # torch.save(model.state_dict(), save_path + 'AntrumPVT.pth')
            torch.save(model.state_dict(), save_path +str(epoch)+ 'EEMPVT-best.pth')
            print('##############################################################################best', best)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_root', type=str,
                        default='/root/autodl-tmp/UGASD_final',
                        help='path to train dataset')
    ...
    logging.basicConfig(filename='GAMNettrain_log.log',
    ...
    train_loader = My_get_loader(opt.data_root,batchsize=opt.batchsize, trainsize=opt.trainsize,
    ...
    for epoch in range(1, opt.epoch+1):
        ...
        train(train_loader, model, optimizer, epoch, opt.data_root)
